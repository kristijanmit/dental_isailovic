...
**Deployment**: prod target is Vercel (plain `next build`, no special config needed — see README §7). `next.config.ts` also conditionally enables `output: "export"` (+ `unoptimized` images) when the `CF_PAGES` env var is set, for a free staging/testing deploy to Cloudflare Pages — see README §7b. This is gated so plain `npm run dev`/`npm run build` are untouched; don't remove the gating without checking both paths still build.
